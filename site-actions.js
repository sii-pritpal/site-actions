                    ...
                    ratings_div.find('i#codebasket_rating_star').mouseenter(function(){
                        ...
                        $.post(main_url+'/common',{action:'save_rating', value:star_number, module_type:module_type, module_id:module_id},function(data){
                            ...
                            if(data['status'] == 'already rated'){
                                alert('You had already given Rating for this...');
                            }
                            if(data['status'] == 'no login'){
        ...
        if(module_type && module_id && main_url) {
            ...
            $.post(ajax_url,{action:'get_comments_information', module_type:module_type, module_id:module_id, limit:limit},function(data){
                ...
                if(data['user_status'] === 1){
                    ...
                    comments_div_html += '<div class="alert alert-danger"><strong>Please login to comment!</strong></div>';
                }
                comments_div_html += '</div><div style="overflow:auto;max-height:400px;border-top:1px solid #cccccc;border-bottom:1px solid #cccccc"><ul data-id="codebasket_comments_list" class="list-group"></ul><a data-id="codebasket_comments_load_button" href="javascript:void(0);" style="display:none" class="btn btn-primary btn-xs btn-block" role="button"><span class="fa fa-refresh"></span> More</a></div><span class="label label-info pull-right" style="margin-top:5px"><span data-id="codebasket_total_comments">'+data['total_comments']+'</span> Comments</span></div>';
    ...
    function put_data_in_comments_list(comment_div, data, prepend){
        ...
        if(data['comments'] != 0){
            $.each(data['comments'], function(k,v){
                if(prepend == true){
                    comments_list.prepend('<li class="list-group-item"><div class="row"><div class="col-xs-2 col-md-2"><img src="'+data['user_images'][v['user_id']]+'" class="img img-rounded img-responsive" alt="No Image" style="margin-top:7px" /></div><div class="col-xs-10 col-md-10"><div><div class="action">'+v.comment+'</div><div class="mic-info">By: <a style="text-decoration:none" id="comment_user_name">'+data['user_names'][v['user_id']]+'</a> <font size="+1">|</font> On: '+data['comment_dates'][v['id']]+' At: '+data['comment_times'][v['id']]+'</div></div></div></div></li>');
                } else {
                    comments_list.append('<li class="list-group-item"><div class="row"><div class="col-xs-2 col-md-2"><img src="'+data['user_images'][v['user_id']]+'" class="img img-rounded img-responsive" alt="No Image" style="margin-top:7px" /></div><div class="col-xs-10 col-md-10"><div><div class="action">'+v.comment+'</div><div class="mic-info"><small style="color:#C0C0C0">By: <a style="text-decoration:none" id="comment_user_name">'+data['user_names'][v['user_id']]+'</a> <font size="+1">|</font> On: '+data['comment_dates'][v['id']]+' At: '+data['comment_times'][v['id']]+'</small></div></div></div></div></li>');
                }
            });
            var this_comments_num = comments_list.find("li").size();
            ...
            if(this_comments_num == 0){
                comments_list.html('<li><div style="padding:10px" class="alert alert-warning"><strong><i class="fa fa-wheelchair"></i> No Comments Yet...</strong></div></li>');
            }
        }
    }
});
