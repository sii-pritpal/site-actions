...
$sql = "CREATE TABLE IF NOT EXISTS `like_dislike` (
  `id` int(250) NOT NULL AUTO_INCREMENT,
  `user_id` int(250) NOT NULL DEFAULT '0',
  `module_type` varchar(250) COLLATE utf8_unicode_ci NOT NULL,
  `module_id` int(250) NOT NULL,
  `liked` enum('0','1') COLLATE utf8_unicode_ci NOT NULL DEFAULT '0',
  `dislike` enum('0','1') COLLATE utf8_unicode_ci NOT NULL DEFAULT '0',
  `created` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `ip` varchar(250) COLLATE utf8_unicode_ci NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
$sql = "CREATE TABLE IF NOT EXISTS `saves` (
  ...
  `module_type` varchar(250) NOT NULL,
  `module_id` int(250) NOT NULL,
  `save` enum('0','1') CHARACTER SET utf8 COLLATE utf8_unicode_ci NOT NULL DEFAULT '0',
  `created` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
  `ip` varchar(250) NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1 AUTO_INCREMENT=1 ;";
...
$sql = "ALTER TABLE  `comments` CHANGE  `status`  `status` ENUM(  '1',  '0' ) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL DEFAULT  '1';";
$database->query($sql);
    ...
    if (!in_array("check_if_liked_or_disliked", $user_defined_funcs)) {
        ...
        function check_if_liked_or_disliked($module_type, $module_id, $database, $user_id, $ip) {
            if ($user_id) {
                ...
                $result = $database->get("like_dislike", "*", array("AND" => array("module_type" => $module_type, "module_id" => $module_id, "ip" => $ip)));
            } else {
                $result = 0;
            }
            return $result;
...
if (isset($_POST['action'])) {
    ...
    if ($action == 'like_module') {
        ...
        if (public_can_like_dislike) {
            ...
            $chk = $database->has("like_dislike", array("AND" => array("module_type" => $module_type, "module_id" => $module_id, "ip" => $ip, "liked" => '1')));
            if ($chk) {
                ...
                $chk = $database->has("like_dislike", array("AND" => array("module_type" => $module_type, "module_id" => $module_id, "ip" => $ip, "dislike" => '1')));
                if ($chk) {
                    //user had disliked the module
                    //change status of module to liked
                    $database->update("like_dislike", array("liked" => '1', "dislike" => '0'), array("AND" => array("module_type" => $module_type, "module_id" => $module_id, "ip" => $ip)));
                } else {
    ...
    if ($action == "save_rating") {//module_type, module_id, value
        ...
        if (public_can_rate == 1) {
            ...
            $chk = $database->has('ratings', array('AND' => array('ip' => $data['ip'], 'module_type' => $module_type, 'module_id' => $module_id)));
            if ($chk) {
